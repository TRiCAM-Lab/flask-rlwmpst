...
window.jsPsych = (function () {
    ...
    var _construct = (function () {
      ...
      if (
        ...
        for (var i = 0; i < parameters.timeline.length; i++) {
          timeline_parameters.timeline.push(
            new TimelineNode(
              Object.assign({}, node_data, parameters.timeline[i]),
              self,
              i
            )
          );
  ...
  function setDefaultValues(trial) {
    var trial_parameters = Object.keys(
      jsPsych.plugins[trial.type].info.parameters
    );
    for (var i = 0; i < trial_parameters.length; i++) {
      if (
        typeof trial[trial_parameters[i]] == "undefined" ||
        trial[trial_parameters[i]] === null
      ) {
        if (
          typeof jsPsych.plugins[trial.type].info.parameters[
            trial_parameters[i]
          ].default == "undefined"
        ) {
          console.error(
...
jsPsych.pluginAPI = (function () {
  ...
  module.getKeyboardResponse = function (parameters) {
    ...
    if (
      ...
      console.log(
        'Invalid RT method specified in getKeyboardResponse. Defaulting to "date" method.'
      );
      parameters.rt_method = "date";
    }
  ...
  module.autoPreload = function (
    ...
    for (var i = 0; i < preloads.length; i++) {
      ...
      for (var j = 0; j < trials.length; j++) {
        if (
          typeof trials[j][param] !== "undefined" &&
          typeof trials[j][param] !== "function"
        ) {
